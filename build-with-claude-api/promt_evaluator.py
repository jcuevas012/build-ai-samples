...
class PromptEvaluator:
    ...
    def generate_unique_ideas(self, task_description, prompt_inputs_spec, num_cases):
        ...
        messages = []
        self.add_user_message(messages, rendered_prompt)
    ...
    def grade_output(self, test_case, output, extra_criteria):
        ...
        * The full 1-10 scale should be utilized - don't hesitate to give low scores when warranted
        ...
        messages = []
        self.add_user_message(messages, eval_prompt)
        self.add_assistant_message(messages, "```json")
        eval_text = self.chat(
            messages,
            stop_sequences=["```"],
    ...
    def run_evaluation(
        ...
        html_output_file="output_evaluation.html",
    ):
        ...
        html = self.report_builder.generate_prompt_evaluation_report(results)
